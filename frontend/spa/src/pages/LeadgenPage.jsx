    ...
    <div className={`${PAGE_INNER} space-y-8 ${PAGE_TEXT}`}>
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-slate-400">
          Загрузите номера в очередь{" "}
          <code className="rounded bg-slate-800 px-1.5 py-0.5 text-emerald-300">dialer_queue</code>
          , затем запустите кампанию — воркер Celery вызовет SIP-адаптер (пока заглушка). Входящие/события от
          АТС — см.{" "}
      ...
      <section>
        <h3 className="mb-2 text-lg font-semibold text-slate-200">Загрузка номеров</h3>
      ...
      <section>
        <h3 className="mb-2 text-lg font-semibold text-slate-200">Запуск кампании</h3>
        <p className="mb-4 text-sm text-slate-400">
          Ставит задачу{" "}
          <code className="rounded bg-slate-800 px-1.5 py-0.5 text-amber-200/90">run_outbound_campaign_task</code> в
          Celery (нужен запущенный worker).
        </p>
        <button
          type="button"
          className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-50"
