  ...
  return (
    <div className="max-w-[100rem] text-slate-100">
      ...
      <p className="mb-4 text-sm leading-relaxed text-slate-300">
        Обзор панели руководителя. Ниже — таблица звонков и результатов ОКК (данные
        подгружаются с бэкенда).
      </p>

      <div className={panelClass}>
        <p className="m-0 text-sm">
          Раздел в разработке: виджеты KPI, последние обращения и статус воркеров.
        </p>
      </div>
      ...
      <p className="mb-4 text-sm text-slate-400">
        Записи сессий после задачи ОКК (Celery): транскрипт в БД, оценка и рекомендации
        от LLM.
      </p>
        ...
        <table className="w-full min-w-[960px] border-collapse text-left">
          <thead>
            <tr className="border-b border-slate-600 bg-slate-900/60">
              <th className={thClass}>Сессия</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                ...
                >
                  Не удалось загрузить данные: {loadError}. Проверьте, что бэкенд
                  доступен через прокси <code className="text-xs">/api/</code>.
                </td>
              </tr>
            ) : items.length === 0 ? (
              <tr>
              ...
              items.map((rec) => {
                ...
                return (
                  ...
                  >
                    <td className={`${tdClass} font-mono text-xs text-sky-300`}>
                      {rec.session_id}
                    </td>
        ...
        <>
          ...
          >
            ...
            <p className="mb-0 border-b border-slate-800 px-4 py-2 text-sm text-slate-400">
              Сессия: {transcriptRec.session_id || "—"} ·{" "}
              {formatDate(transcriptRec.created_at)}
            </p>
